...
def get_participants_of_course(course_id):
    sql = "SELECT users.username, users.id, roles.name FROM users \
...
def enroll_course(course_id):
    try:
        ...
        if user_enrolled(session["user_id"],course_id) == None:
            sql = "INSERT INTO participants (user_id, course_id) VALUES (:user_id,:course_id)"
            db.session.execute(sql, {"user_id": session["user_id"], "course_id": course_id})
            db.session.commit()
            return True
        else:
            return False
...
def user_enrolled(user_id, course_id):
    sql = "SELECT user_id FROM participants WHERE user_id=:user_id AND course_id=:course_id"
